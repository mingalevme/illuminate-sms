...
class SmscTest extends TestCase
{
    ...
    {
        $login = strval(getenv('MINGALEVME_TEST_ILLUMINATE_SMS_SMSC_LOGIN'));
        $password = strval(getenv('MINGALEVME_TEST_ILLUMINATE_SMS_SMSC_PASSWORD'));
        $phone = strval(getenv('MINGALEVME_TEST_ILLUMINATE_SMS_PHONE'));

        if (empty($login) || empty($password) || empty($phone)) {
            $this->markTestSkipped('To test the class set up MINGALEVME_TEST_ILLUMINATE_SMS_* environment variables');
        }
